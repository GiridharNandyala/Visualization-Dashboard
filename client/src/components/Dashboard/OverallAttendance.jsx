...
const OverallAttendance = () => {
  ...
  const processOverallAttendance = (data) => {
    if (data && data.length > 1) {
      ...
      const nameColumnIndex = 3; // Assuming the second column contains student names
      const attendanceCounts = { below65: 0, between65And75: 0, above75: 0 };
      const studentNames = { below65: [], between65And75: [], above75: [] };
      ...
      data.slice(6).forEach((row) => { // Skip the header row
        ...
        if (!isNaN(total) && studentName) {
          if (total < 65) {
            attendanceCounts.below65++;
            studentNames.below65.push(studentName);
          } else if (total >= 65 && total <= 75) {
            attendanceCounts.between65And75++;
            studentNames.between65And75.push(studentName);
          } else {
            attendanceCounts.above75++;
            studentNames.above75.push(studentName);
          }
        }
      });
